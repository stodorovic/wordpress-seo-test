...
class WPSEO_OpenGraph_Test extends WPSEO_UnitTestCase {
	...
	public function test_image_IS_SINGULAR_and_HAS_open_graph_image() {
		$post_id = $this->factory->post->create();
		$image   = get_site_url() . '/wp-content/plugins/wordpress-seo-test/integration-tests/assets/small.png';
	...
	public function test_image_IS_SINGULAR_and_HAS_open_graph_image_AND_HAS_content_images() {
		$post_id = $this->factory->post->create(
			array(
				'post_content' => '<img class="alignnone size-medium wp-image-490" src="' . get_site_url() . '/wp-content/plugins/wordpress-seo-test/integration-tests/yoast.png" />',
			)
		);
		...
		$expected_output = '<meta property="og:image" content="' . get_site_url() . '/wp-content/plugins/wordpress-seo-test/integration-tests/yoast.png" />';
